...
def evaluate_applicant(applicant):
    try:
        ...
        compensation_ok = (
            isinstance(rate, (int, float)) and 
            isinstance(availability, (int, float)) and 
            rate <= 100 and availability >= 20
        )
        ...
        if experience_ok and compensation_ok and location_ok:
            ...
            try:
                ...
                print(f"❌ Error writing shortlist data for {applicant_id}: {e}")
        else:
            print(f"❌ Not shortlisted: {applicant_id}")

    except Exception as e:
        print(f"❌ Unexpected error for applicant {applicant.get('id', 'unknown')}: {e}")
